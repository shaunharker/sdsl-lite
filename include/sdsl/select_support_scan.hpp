...
class select_support_scan : public select_support<t_bitvector>
{
    private:
    ...
    public:
        ...
        void load(std::istream&, SDSL_UNUSED const t_bitvector* v=nullptr) {
            set_vector(v);
        }
        ...
        select_support_scan<t_b, t_pat_len, t_bitvector>& operator=(const select_support_scan& ss) {
            set_vector(ss.m_v);
            return *this;
        }
        void swap(select_support_scan<t_b, t_pat_len, t_bitvector>&) {}
};

template<uint8_t t_b, uint8_t t_pat_len, class t_bitvector>
inline typename select_support_scan<t_b,t_pat_len,t_bitvector>::size_type select_support_scan<t_b,t_pat_len,t_bitvector>::select(size_type i)const
{
    auto data = m_v->data();
    ...
    return (word_pos<<6) + select_support_trait<t_b,t_pat_len,t_bitvector>::ith_arg_pos_in_the_word(*data, i-sum_args, old_carry);
}
